...
def test_deploy_requires_superuser(auth_client):
...
@pytest.mark.django_db
def test_deploy_rejects_plain_staff(staff_client, tmp_path, settings):
...
def test_deploy_writes_flag_file(superuser_client, tmp_path, settings):
    ...
    response = superuser_client.post(reverse("deploy"), HTTP_REFERER="/somewhere/")
...
@pytest.mark.django_db
def test_deploy_without_configured_flag_file_shows_error(superuser_client, settings):
    settings.DEPLOY_FLAG_FILE = ""

    response = superuser_client.post(reverse("deploy"))
...
@pytest.mark.django_db
def test_deploy_falls_back_to_index_without_referer(
...
@pytest.mark.django_db
def test_deploy_button_visible_in_nav_for_superuser(
    superuser_client, tmp_path, settings
):
...
@pytest.mark.django_db
def test_deploy_button_hidden_for_plain_staff(staff_client, tmp_path, settings):
    settings.DEPLOY_FLAG_FILE = str(tmp_path / "deploy.flag")
