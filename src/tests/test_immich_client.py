...
def test_upload_asset_builds_multipart_fields_and_returns_created(
    request_mock, client, tmp_path
):
    file_path = tmp_path / "slide.jpg"
    file_path.write_bytes(b"\xff\xd8jpeg-bytes")
...
def test_upload_asset_returns_duplicate_status(request_mock, client, tmp_path):
    file_path = tmp_path / "slide.jpg"
...
def test_upload_asset_sends_checksum_header_when_provided(
    request_mock, client, tmp_path
):
    file_path = tmp_path / "slide.jpg"
    file_path.write_bytes(b"data")
